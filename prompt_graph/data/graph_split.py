...
def graph_split(graph_list, shot_num):
    ...
    for data in graph_list:
        ...
        class_datasets[label].append(data)    # 这样做是确保了每个label都能得到shot_num个数据

    train_data = []
    remaining_data = []
    for label, data_list in class_datasets.items(): #主要就是在这里，遍历了每个label下的数据，取出shot_num作为训练集
        train_data.extend(data_list[:shot_num])     #可以看到训练集的概念是和shot_num绑定的

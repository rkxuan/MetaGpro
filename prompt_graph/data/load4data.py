...
def node_sample_and_save(data, k, folder, num_classes):    
    ...
    # 从剩下的数据中选出k*标签数个样本作为训练集,k-way n-shot
...
def node_sample_and_save_(data, k, folder, num_classes, train_mask):
    # 请注意这里假设了下游任务已经知道一个图中哪些节点是lebeled, 哪些并不是, 而上面那个函数是随机采样labeled数据
    
    train_mask = train_mask.to('cpu')
    labels = data.y.to('cpu')
    idx = torch.arange(0, data.num_nodes)

    test_idx = idx[train_mask==0]
    test_labels = labels[train_mask==0]

    remaining_idx = idx[train_mask==1]
    remaining_labels = labels[remaining_idx]
    ...
    train_mask[train_idx] = 0    # 相当于 After you sample from a list, set replace=False
    ...
    torch.save(test_idx, os.path.join(folder, 'test_idx.pt'))
    torch.save(test_labels, os.path.join(folder, 'test_labels.pt'))
    print("save file path is:", folder)
    return train_mask
...
def load4graph(dataset_name, shot_num= 10, num_parts=None, pretrained=False):
    ...
    if dataset_name in ['MUTAG', 'ENZYMES', 'COLLAB', 'PROTEINS', 'IMDB-BINARY', 'REDDIT-BINARY', 'COX2', 'BZR', 'PTC_MR', 'DD']:
        dataset = TUDataset(root='/root/autodl-tmp/TUDataset', name=dataset_name, use_node_attr=True)  # use_node_attr=False时，节点属性为one-hot编码的节点类别
    ...
    if dataset_name in ['ogbg-ppa', 'ogbg-molhiv', 'ogbg-molpcba', 'ogbg-code2']:
        dataset = PygGraphPropPredDataset(name = dataset_name, root='/root/autodl-tmp/dataset')
        input_dim = dataset.num_features
        out_dim = dataset.num_classes
...
def load4node(dataname):
    ...
    base_transform_list = [T.AddSelfLoops()]
    Undirected_transform_list = [T.AddSelfLoops(), T.ToUndirected()]
    if dataname in ['PubMed', 'CiteSeer', 'Cora']:
        transform_list = Undirected_transform_list + [T.NormalizeFeatures()]
        transform = T.Compose(transform_list)
        dataset = Planetoid(root='/root/autodl-tmp/Planetoid', name=dataname, transform=transform)
        data = dataset[0]
        input_dim = dataset.num_features
        out_dim = dataset.num_classes
        ...
        dataset = Reddit(root='/root/autodl-tmp/Reddit')
        data = dataset[0]
        input_dim = dataset.num_features
        out_dim = dataset.num_classes
    elif dataname == 'WikiCS':
        dataset = WikiCS(root='/root/autodl-tmp/WikiCS')
        data = dataset[0]
        input_dim = dataset.num_features
        out_dim = dataset.num_classes
    elif dataname == 'Flickr':
        dataset = Flickr(root='/root/autodl-tmp/Flickr')
        data = dataset[0]
        input_dim = dataset.num_features
        out_dim = dataset.num_classes
    elif dataname in ['Wisconsin', 'Texas']:
        dataset = WebKB(root='/root/autodl-tmp/'+dataname, name=dataname)
        data = dataset[0]
        input_dim = dataset.num_features
        out_dim = dataset.num_classes
    elif dataname == 'Actor':
        dataset = Actor(root='/root/autodl-tmp/Actor')
        data = dataset[0]
        input_dim = dataset.num_features
        out_dim = dataset.num_classes
    elif dataname == 'ogbn-arxiv':
        dataset = PygNodePropPredDataset(name='ogbn-arxiv', root='/root/autodl-tmp/dataset')
        data = dataset[0]
        input_dim = data.x.shape[1]
        out_dim = dataset.num_classes
    elif dataname in ['ENZYMES', 'PROTEINS']:
        # 实现TUDataset中两个multi graphs dataset的节点分类
        dataset = TUDataset(root='/root/autodl-tmp/TUDataset', name=dataname, use_node_attr=True)
...
def load4node_to_sparse(dataname):
    base_transform_list = [T.AddSelfLoops(), T.ToSparseTensor()]
    Undirected_transform_list = [T.AddSelfLoops(), T.ToUndirected(), T.ToSparseTensor()]
    if dataname in ['PubMed', 'CiteSeer', 'Cora']:
        transform_list = Undirected_transform_list + [T.NormalizeFeatures()]
        transform = T.Compose(transform_list)
        dataset = Planetoid(root='/root/autodl-tmp/Planetoid', name=dataname, transform=transform)
        ...
        dataset = Reddit(root='/root/autodl-tmp/Reddit', transform=T.ToSparseTensor)
        data = dataset[0]
        input_dim = dataset.num_features
        out_dim = dataset.num_classes
    elif dataname == 'WikiCS':
        dataset = WikiCS(root='/root/autodl-tmp/WikiCS', transform=T.ToSparseTensor)
        data = dataset[0]
        input_dim = dataset.num_features
        out_dim = dataset.num_classes
    elif dataname == 'Flickr':
        dataset = Flickr(root='/root/autodl-tmp/Flickr', transform=T.ToSparseTensor)
        data = dataset[0]
        input_dim = dataset.num_features
        out_dim = dataset.num_classes
    elif dataname in ['Wisconsin', 'Texas']:
        dataset = WebKB(root='/root/autodl-tmp/'+dataname, name=dataname, transform=T.ToSparseTensor)
        data = dataset[0]
        input_dim = dataset.num_features
        out_dim = dataset.num_classes
    elif dataname == 'Actor':
        ...
        dataset = PygNodePropPredDataset(name='ogbn-arxiv', root='/root/autodl-tmp/dataset', transform=T.ToSparseTensor)
        data = dataset[0]
        input_dim = data.x.shape[1]
        out_dim = dataset.num_classes
    elif dataname in ['ENZYMES', 'PROTEINS']:
        # 实现TUDataset中两个multi graphs dataset的节点分类
        dataset = TUDataset(root='/root/autodl-tmp/TUDataset', name=dataname, use_node_attr=True, transform=T.ToSparseTensor)
        node_class = dataset.data.x[:,-3:]
        input_dim = dataset.num_node_features
        out_dim = dataset.num_node_labels
        data = Batch.from_data_list(dataset)  # 将dataset中小图合并成一个大图
...
def load4link_prediction_multi_graph(dataset_name, num_per_samples=1):
    if dataset_name in ['MUTAG', 'ENZYMES', 'COLLAB', 'PROTEINS', 'IMDB-BINARY', 'REDDIT-BINARY', 'COX2', 'BZR', 'PTC_MR', 'DD']:
        dataset = TUDataset(root='/root/autodl-tmp/TUDataset', name=dataset_name, use_node_attr=True)

    if dataset_name in ['ogbg-ppa', 'ogbg-molhiv', 'ogbg-molpcba', 'ogbg-code2']:
        dataset = PygGraphPropPredDataset(name = dataset_name, root='/root/autodl-tmp/dataset')
    
    input_dim = dataset.num_features
    output_dim = 2 # link prediction的输出维度应该是2，0代表无边，1代表有边
